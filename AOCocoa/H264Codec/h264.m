...
static  void mc_dir_part(H264Context *h, MpegPicture *pic, int n, int square, int chroma_height, int delta, int list,
...
static int fill_default_ref_list(H264Context *h)
{
    MpegEncContext * const s = &h->s;
    int i;
    MpegPicture sorted_short_ref[16];
    ...
    if(s->picture_structure == PICT_FRAME)
	{
        if(h->slice_type==B_TYPE)
		{
            ...
            for(list=0; list<2; list++)
			{
				...
				{
                    MpegPicture temp= h->default_ref_list[1][0];
                    h->default_ref_list[1][0] = h->default_ref_list[1][1];
                    h->default_ref_list[1][0] = temp;
                }
				
                if(index < h->ref_count[ list ])
                    memset(&h->default_ref_list[list][index], 0, sizeof(MpegPicture)*(h->ref_count[ list ] - index));
            }
        }
		else
		{
            ...
            if(index < h->ref_count[0])
                memset(&h->default_ref_list[0][index], 0, sizeof(MpegPicture)*(h->ref_count[0] - index));
        }
    }
...
static int decode_ref_pic_list_reordering(H264Context *h)
{
    ...
    for(list=0; list<2; list++)
	{
        memcpy(h->ref_list[list], h->default_ref_list[list], sizeof(MpegPicture)*h->ref_count[list]);
        ...
        if(get_bits1(&s->gb))
		{
            ...
            for(index=0; ; index++)
			{
                ...
                if(reordering_of_pic_nums_idc<3)
				{
                    ...
                    if(i < index)
					{
                        memset(&h->ref_list[list][index], 0, sizeof(MpegPicture)); //FIXME
                    }
					else if(i > index)
					{
                        MpegPicture tmp= h->ref_list[list][i];
                        for(; i>index; i--)
						{
...
static MpegPicture * remove_short(H264Context *h, int frame_num)
{
    int i;
		
    for(i=0; i<h->short_ref_count; i++)
	{
        MpegPicture *pic= h->short_ref[i];
		
        if(pic->frame_num == frame_num)
		{
            h->short_ref[i]= NULL;
            memmove(&h->short_ref[i], &h->short_ref[i+1], (h->short_ref_count - i - 1)*sizeof(MpegPicture*));
            h->short_ref_count--;
            return pic;
        }
    }
...
static MpegPicture * remove_long(H264Context *h, int i)
{
    MpegPicture *pic;
    ...
    h->long_ref[i]= NULL;
    memmove(&h->long_ref[i], &h->long_ref[i+1], (h->long_ref_count - i - 1)*sizeof(MpegPicture*));
    h->long_ref_count--;
...
//			assert(0);
    ...
    if(!current_is_long)
	{
        ...
        if(h->short_ref_count)
            memmove(&h->short_ref[1], &h->short_ref[0], h->short_ref_count*sizeof(MpegPicture*));
